                ...
                foreach ($userdetails as $value) {
                    ...
                    if ($temp->status != 'A') {
                        if ($value->status == 'A' && strtotime($value->first_lesson_date) == strtotime($date)) {
                            ...
                            $data['userdetails'][] = array('id' => $temp->id, 'firstname' => $temp->firstname, 'lastname' => $temp->lastname, 'attadence' => $attadence->attendance, 'attadence_id' => $attadence->id, 'clan' => $clan->{$this->session_data->language . '_class_name'}, 'school' => $clan->School->{$this->session_data->language . '_school_name'}, 'academy' => $clan->School->Academy->{$this->session_data->language . '_academy_name'}, 'type' => 'trial');
                        }
                    } else {
                        if (!is_null($temp->id)) {
                            $attadence = new Attendance();
            ...
            if ($obj_recover->result_count() > 0) {
                foreach ($obj_recover as $student) {
                    ...
                    if ($user->result_count() == 1) {
                        ...
                        //Set an array of user details for view part
                        $data['userdetails'][] = array('id' => $user->id, 'firstname' => $user->firstname, 'lastname' => $user->lastname, 'attadence' => $recover->attendance, 'attadence_id' => $recover->attendance_id, 'clan' => $clan->{$this->session_data->language . '_class_name'}, 'school' => $clan->School->{$this->session_data->language . '_school_name'}, 'academy' => $clan->School->Academy->{$this->session_data->language . '_academy_name'}, 'type' => 'recover');
                    }
                }
            }
